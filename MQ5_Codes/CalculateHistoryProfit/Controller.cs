...
namespace MtGuiControllerForm
{
    ...
    public partial class GuiController
    {
        ...
        private void OnListBoxSelectionChanged(object sender, EventArgs e)
        {
            ...
            string selectedItems = string.Join(",", control.SelectedItems.Cast<string>());

            GuiEvent evnt = new GuiEvent
            {
                id = GuiEventType.ListBoxSelect,  // Создаем новый тип события для ListBox
                el_name = control.Name,
                sparam = selectedItems, // Передаем выбранные элементы как строку
                lparam = control.SelectedIndex
            };
            m_events.Add(evnt);
        }
        ...
        private void SubscribeOnElements(Form form)
        {
            ...
            types_and_events.Add(typeof(ListBox), new List<HandlerControl>()
                {
                    listbox => ((ListBox)listbox).SelectedIndexChanged += OnListBoxSelectionChanged
                });
